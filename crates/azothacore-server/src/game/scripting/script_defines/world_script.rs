...
impl ScriptObjectTrait for WorldScriptObject {
    fn remove_systems_from_bevy(&self, bevy_world: &mut World) {
        _ = self.on_open_state_change.map(|s| bevy_world.remove_system(s));
        _ = self.on_after_config_load.map(|s| bevy_world.remove_system(s));
        _ = self.on_load_custom_database_table.map(|s| bevy_world.remove_system(s));
        _ = self.on_before_config_load.map(|s| bevy_world.remove_system(s));
        _ = self.on_motd_change.map(|s| bevy_world.remove_system(s));
        _ = self.on_shutdown_initiate.map(|s| bevy_world.remove_system(s));
        _ = self.on_shutdown_cancel.map(|s| bevy_world.remove_system(s));
        _ = self.on_update.map(|s| bevy_world.remove_system(s));
        _ = self.on_startup.map(|s| bevy_world.remove_system(s));
        _ = self.on_shutdown.map(|s| bevy_world.remove_system(s));
        _ = self.on_after_unload_all_maps.map(|s| bevy_world.remove_system(s));
        _ = self.on_before_finalize_player_world_session.map(|s| bevy_world.remove_system(s));
        _ = self.on_before_world_initialized.map(|s| bevy_world.remove_system(s));
    }
}
