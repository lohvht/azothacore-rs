...
impl ScriptObjectTrait for AccountScriptObject {
    fn remove_systems_from_bevy(&self, bevy_world: &mut World) {
        _ = self.on_account_login.map(|s| bevy_world.remove_system(s));
        _ = self.on_last_ip_update.map(|s| bevy_world.remove_system(s));
        _ = self.on_failed_account_login.map(|s| bevy_world.remove_system(s));
        _ = self.on_email_change.map(|s| bevy_world.remove_system(s));
        _ = self.on_failed_email_change.map(|s| bevy_world.remove_system(s));
        _ = self.on_password_change.map(|s| bevy_world.remove_system(s));
        _ = self.on_failed_password_change.map(|s| bevy_world.remove_system(s));
        _ = self.can_account_create_character.map(|s| bevy_world.remove_system(s));
    }
}
