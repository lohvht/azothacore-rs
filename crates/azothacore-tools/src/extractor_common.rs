...
use azothacore_common::{az_error, AzResult, Locale};
...
use crate::{
    extractor_common::casc_handles::{CascHandlerError, CascLocale, CascStorageHandle},
    to_casc_locales,
};
...
impl ExtractorConfig {
    pub fn get_casc_storage_handler(&self, locale: Locale) -> Result<CascStorageHandle, CascHandlerError> {
        CascStorageHandle::build(self.input_storage_data_dir(), to_casc_locales(&locale))
    }
