...
structstruck::strike! {
  ...
  pub struct WorldserverConfig {
    ...
    #[serde(default="default_worldserver_log_appenders")]
    pub Appender: Vec<LogAppender>,
    #[serde(default="default_worldserver_log_configs")]
    pub Logger: Vec<LogLoggerConfig>,
...
pub fn default_worldserver_log_configs() -> Vec<LogLoggerConfig> {
    ...
    vec![
        ...
        LogLoggerConfig {
            name:      String::from("scripts::hotswap"),
            min_level: Info,
            max_level: Error,
            appenders: vec![String::from("Console"), String::from("Server")],
        },
...
mod tests {
    ...
    use crate::{configuration::*, workspace_dir};
    ...
    fn it_reads_the_worldserver_toml_dist_file() {
        let dist = match toml_from_filepath::<WorldserverConfig, _>(workspace_dir!().join("env/dist/etc/app-worldserver.dist.toml")) {
            Err(ConfigError::TOMLDecode { filepath, err }) => {
                let s = fs::read_to_string(filepath).unwrap();
                panic!("err_msg: '{}', error at '{}'", err.message(), &s[err.span().unwrap()]);
            },
            Err(e) => panic!("{}", e),
            Ok(d) => d,
        };
