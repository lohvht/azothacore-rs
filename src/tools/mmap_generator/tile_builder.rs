...
use crate::{
    ...
    server::shared::recastnavigation_handles::{detour_create_nav_mesh_data, DetourNavMesh, DetourNavMeshParams, RecastConfig},
...
impl TileBuilder {
    ...
    pub fn build_move_map_tile(
        ...
        if iv_poly_mesh.is_null() {
            return Err("alloc iv.polyMesh FAILED!".into());
        }
        ...
        if iv_poly_mesh_detail.is_null() {
            return Err("alloc m_dmesh FAILED!".into());
        }
